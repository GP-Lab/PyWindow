...
class MainWindow(QMainWindow, Ui_MainWindow, Ui_Form):
    def __init__(self):
        ...
        # 点击菜单栏的Export,输出窗口参数
        self.actionExport.triggered.connect(self.export_window)
    ...
    # 将窗函数保存到文件
        # 获取选中的窗口
        window_name = self.listWidget.currentItem().text()
        # 获取窗口
        if self.comboBox.currentText() == 'kaiser':
            window = get_window((self.window[window_name]['type'], float(self.lineEdit_2.text())),
                                int(self.window[window_name]['length']))
        else:
            if self.window[window_name]['s_mode'] == 'symmetric':
                s_mode = False
            elif self.window[window_name]['s_mode'] == 'periodic':
                s_mode = True
            window = get_window(self.window[window_name]['type'], int(self.window[window_name]['length']), fftbins=s_mode)
        ...
        with open(save_path, 'w') as f:
            ...
            f.write('Window name: ' + window_name + '\n')
            f.write('Window type: ' + self.window[window_name]['type'] + '\n')
            f.write('Window length: ' + str(self.window[window_name]['length']) + '\n')
            f.write('Window symmetric mode: ' + self.window[window_name]['s_mode'] + '\n')
            f.write('\n')
